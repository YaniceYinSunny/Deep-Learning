...
class Attention:
        ...
        def forward(self, X):
            ...
            attn_mask    = torch.ones_like(self.A_w).triu(diagonal=1) # TODO
            # convert to negative infinity
            attn_mask    = attn_mask * -1e9

            # Calculate/normalize Attention Scores

            self.A_sig   = self.softmax.forward((self.A_w + attn_mask) / torch.sqrt(torch.tensor(self.Q.shape[-1]).float()))
        ...
        def backward(self, dLdXnew):
            ...
            dLdA_w         = self.softmax.backward(dLdA_sig) / torch.sqrt(torch.tensor(self.Q.shape[-1]).float())
            ...
            self.dLdWq     = self.X.permute(0, 2, 1) @ self.dLdQ
            self.dLdWv     = self.X.permute(0, 2, 1) @ self.dLdV
            self.dLdWk     = self.X.permute(0, 2, 1) @ self.dLdK
